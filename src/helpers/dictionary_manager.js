...
export default class DictionaryManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (fs.existsSync(SUPPORTED_LANGUAGES_PATH())) {
        if (!DictionaryManager.isFileExpired(SUPPORTED_LANGUAGES_PATH())) {
          ...
          const jsonStringFromFile = fs.readFileSync(
            SUPPORTED_LANGUAGES_PATH()
          );
          ...
          if (
            isObject(supportedLanguagesJsonParsed) &&
            Array.isArray(supportedLanguagesJsonParsed)
          ) {
      ...
      https
        .get(requestOptions, (response) => {
          if (response.statusCode === 200 || response.statusCode === 302) {
            ...
            let supportedLanguagesJsonFile = fs.createWriteStream(
              SUPPORTED_LANGUAGES_PATH()
            );
  ...
  static doesLanguageExistForLocale(userLanguage, supportedLocales) {
    ...
    if (listOfSupportedLanguages.includes(userLanguage)) {
      // language has an exact match and is supported
      downloadDictionaryKey = userLanguage;
    } else if (userLanguage in specialLanguageCases) {
      // language is a special case and is supported
  ...
  static async getLanguagePath(userLanguage, localeKey) {
    return new Promise((resolve, reject) => {
      const localDictionaryFiles = {
        userLanguageAffFile: path.join(
          SPELLING_DICTIONARIES_PATH(),
          `${userLanguage}.aff`
        ),
        userLanguageDicFile: path.join(
          SPELLING_DICTIONARIES_PATH(),
          `${userLanguage}.dic`
        ),
      };
      const languageDictFilesExist =
        fs.existsSync(localDictionaryFiles.userLanguageAffFile) &&
        fs.existsSync(localDictionaryFiles.userLanguageDicFile);
      const languageDictFilesTooOld =
        languageDictFilesExist &&
        DictionaryManager.isFileExpired(
          localDictionaryFiles.userLanguageAffFile
        ); // Only need to check one of the two
      if (languageDictFilesExist && !languageDictFilesTooOld) {
        ...
        if (localeKey) {
          ...
          https
            .get(`${dictBaseUrl}.aff`, (response) => {
              if (response.statusCode === 200 || response.statusCode === 302) {
                let affFile = fs.createWriteStream(
                  localDictionaryFiles.userLanguageAffFile
                );
                ...
                affFile.on("finish", (finished) => {
                  ...
                  downloadState.affFile &&
                    downloadState.dicFile &&
                    resolve(localDictionaryFiles);
                });
              }
            })
          ...
          https
            .get(`${dictBaseUrl}.dic`, (response) => {
              if (response.statusCode === 200 || response.statusCode === 302) {
                let dicFile = fs.createWriteStream(
                  localDictionaryFiles.userLanguageDicFile
                );
